// Create a script to add a template directly to the database
...
async function main() {
  try {
    ...
    const verifyTemplate = await prisma.template.findUnique({
      where: { id: template.id },
      include: { sections: true }
    });
    
    if (verifyTemplate) {
      console.log('Successfully verified template exists in the database');
      console.log(`Template has ${verifyTemplate.sections.length} sections`);
    } else {
      console.log('Failed to verify template - could not find it after creation');
    }
